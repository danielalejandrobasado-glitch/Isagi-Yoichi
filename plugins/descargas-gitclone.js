...
let handler = async (m, { args, usedPrefix, command }) => {
  if (!args[0]) {
    return conn.reply(m.chat, `🎤💙 Por favor, ingresa la URL de un repositorio de GitHub que deseas descargar en el mundo virtual ✨🎵`, m)
  }
  if (!regex.test(args[0])) {
    return conn.reply(m.chat, `🎤💙 ¡Gomen! Verifica que la *URL* sea de GitHub válida para el concierto virtual ✨💫`, m).then(_ => m.react('💙'))
  }
  ...
  await m.react('🎤')
  try {
  conn.reply(m.chat, '🎤💙 Descargando del ciberespacio virtual... ✨🎵', m)
