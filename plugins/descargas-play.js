...
author = author || 'no encontrado'
    ...
    if (command === 'play' || command === 'yta' || command === 'ytmp3' || command === 'playaudio') {
      try {
        ...
        return conn.reply(m.chat, '🎵💙 ¡Gomen nasai! No se pudo enviar el audio virtual. Esto puede deberse a que el archivo es demasiado pesado o a un error en la generación de la URL. Por favor, intenta nuevamente más tarde ✨', m)
      }
      ...
      try {
        ...
        return conn.reply(m.chat, '🎤💫 ¡Gomen! No se pudo enviar el video virtual. Esto puede deberse a que el archivo es demasiado pesado o a un error en la generación de la URL. Por favor, intenta nuevamente más tarde 🎵', m)
      }
    } else {
      return conn.reply(m.chat, '🎶💙 Comando musical no reconocido ✨', m)
    }
  } catch (error) {
